...
[![Docs](https://img.shields.io/badge/Docs-Bitfinex.Net-1b7f50?style=for-the-badge)](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=Bitfinex.Net)
...
The [Bitfinex.Net documentation](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=Bitfinex.Net) is the main resource for installing, configuring, and using the library.

| Resource | Description |
|--|--|
| [Client guide](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=Bitfinex.Net) | Installation, REST and WebSocket clients, authentication, dependency injection, error handling, and advanced features |
| [Examples](https://cryptoexchange.jkorf.dev/docs/exchange-clients/examples?library=Bitfinex.Net) | Common REST and WebSocket operations |
| [API reference](https://cryptoexchange.jkorf.dev/docs/exchange-clients/reference?library=Bitfinex.Net) | Client interfaces, methods, and properties |
...
For more examples and explanations, continue with the [Bitfinex.Net documentation](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=Bitfinex.Net) or browse the [compilable repository examples](https://github.com/JKorf/Bitfinex.Net/tree/master/Examples).
...
V2 uses a strict interface per capability. The `SharedApi` property on each API surface therefore exposes only the operations or subscriptions that surface actually supports. `IBitfinexSharedApiClient` groups the exchange's Shared API surfaces for dependency injection and runtime capability lookup.
...
using var restClient = new BitfinexRestClient();
...
The request and response models belong to `CryptoExchange.Net.SharedApis`, so the same operation can accept another exchange's `IGetTickerRest` implementation. When using dependency injection, inject `IBitfinexSharedApiClient` to access all of the exchange's Shared API surfaces or inject a capability such as `IGetTickerRest` directly. Use `GetCapability` on the aggregate when the operation, transport, or trading mode is selected at runtime.
...
## Release notes
* Version 11.6.0 - 24 Sep 2026
    * Updated CryptoExchange.Net to v13.0.0
    * Shared APIs
      * Added SharedApi V2 support
      * Added `.SharedApi` property to clients
      * Added `(I)BitfinexSharedApiClient` containing all V2 SharedApi references
      * Added `SharedApi` to `BitfinexOptions` to provide SharedApi related options
    ...
    * Added ExchangeApi.Account.GetAccountSecurityAsync endpoint
...
* Version 11.0.0 - 29 Jun 2026
    ...
    * Shared APIs:
      * Added IFuturesOrderRestClient and IFuturesTickerRestClient to REST shared implementations
      * Added IPositionSocketClient, IFuturesOrderSocketClient to Socket shared implementations
      ...
      * Updated Shared logic to correctly filter between Spot and Futures data
    ...
    * Added restClient.ExchangeApi.Account.GetSymbolMarginInfoSymbolsAsync endpoint
    * Added MarginRequired property to BitfinexMarginBaseInfo model
    * Added travel rule parameters to WithdrawV2Async
    ...
    * Added setter to BitfinexExchange.RateLimiter to allow custom rate limit settings
    ...
    * Fixed code comments documentation references
    * Fixed restClient.ExchangeApi.ExchangeData.GetDerivativesStatusHistoryAsync deserialization
