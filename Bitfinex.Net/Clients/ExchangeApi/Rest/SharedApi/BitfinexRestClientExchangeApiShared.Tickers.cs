            ...
            return HttpResult.Ok<SharedTicker[]>(result, data.Select(x =>
            {
                if (!x.Symbol.Contains("F0"))
                {
                    return new SharedSpotTicker(
                        ...
                        Math.Round(x.DailyChangePercentage * 100, 2));
                }
